...
```{r}
plot(game_by_game15_16$fg3m ~ game_by_game15_16$fg3_pct, col=c("firebrick", "blue"), pch=16, main="3 Pointers made vs. 3 point percentage")

plot(game_by_game15_16$fgm ~ game_by_game15_16$ast, col=c("firebrick", "blue"), pch=16, main="Field goals made vs. assists")

plot(game_by_game15_16$fg3a ~ game_by_game15_16$fg3m, col=c("firebrick", "blue"), pch=16, main="3 pointers attempted vs. 3 pointers made")
```
...
The model below uses some stats from a game by game of each team, we train on previous seasons and make our predictions on the current season. We can see our success rate is at about 81% win rate. Also we created conf. matrix to and ploted it.
```{r}
...
#1st Round Predictions for the First model
...
##1st Round Predictions for the second model
...
##2nd Round Predictions for the first model
Winners are:
...
```
##2nd Round Predictions for the second model
Winners are:
...
set.seed(12)
series_2("warriors","clippers")

series_2("thunder","spurs")

series("pistons","hawks")

series("hornets","raptors")

```


##3rd Round Predictions for the first model
Winners are:
...
##3rd Round Predictions for the second model
Winners are:
...
series_2("pistons","hornets")

```


##Final Round Predictions for the first model
Winner is:
...
```{r}
set.seed(12)
series_2("warriors","pistons")

```

##Final Thoughts

Thought both models predict the Warriors win the championship, the first model does a better job of predicting closer to what is actually happening in the playoffs.

<a href="http://www.nba.com/playoffs/">Playoff Bracket Click Here</a>